      ...
      <div class="login-box pr">
        <i-form :model="loginItem" class="iform" :rules="ruleInline" ref="loginItem">
          <form-item prop="language" class="top-item flex">
            <radio-group v-model="languageType">
...
export default {
  ...
  computed: {
    languageCtx () {
      console.log(language, this.languageType)
      return language[this.languageType].login
    },
    isRemaindMicroRight: {
      get () {
  ...
  methods: {
    initLanguage () {
      ...
      if (mylanguage && this.loginItem.user !== '' && mylanguage[this.loginItem.user]) {
        console.log(mylanguage)
        this.languageType = mylanguage[this.loginItem.user]
      }
    },
    initAccountInfo() {
    ...
    handleSubmit(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          this.isLoading = true;
          this.$store
            .dispatch(types.GetUserInfo, { account, password, language: this.languageType})
            .then(res => {
