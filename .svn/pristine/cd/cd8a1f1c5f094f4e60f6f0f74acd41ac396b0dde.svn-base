    ...
    dispatchSwitchGroup (item) {
      ...
      if (this.activeGid === item.gid) {
        if (!this.tempGroupInfo || (this.tempGroupInfo && this.tempGroupInfo.creatType === 'SINGLE_TEMP_GROUP')) { // 没有临时群组或者在当前组有单呼
          ...
          this.$emit('on-change', 'nowGroup')
        }
      } else {
        this.$store.dispatch(userTypes.SwitchGroup, item)
        .then(res => {
          this.messageNotice(8) // 切换成功
            ...
            .then((memberList) => {
              ...
              if (this.tempGroupInfo && this.tempGroupInfo.creatType !== 'SINGLE_TEMP_GROUP') {
                this.$emit('on-change', 'otherGroup')
                this.nowStatus = this.languageCtx.blockGroup.tempGroup
              }
