...
import {receiveNotice, queryTempGroup, webSocket, receiveVoice, saveVoice, receiveSMS, receiveGPS, hasRightOfMicro, getRecorder, isReconnect, pttStop} from '@/libs/webDispatcher-sdk.js'
...
export default {
  ...
  mounted () {
    getRecorder().then(() => { // 获取语音权限
    }).catch((res) => {
      this.microTip = true
    })
    this.initData()
    document.addEventListener('visibilitychange', () => { // 鼠标离开当前页断掉ptt
      document.hidden && this.handleEndPtt()
    })
  },
  methods: {
    initData () {
    ...
    initLanguage () {
      let mylanguage = Storage.localGet('language')
      if (mylanguage) {
        this.$store.commit(user.SetLanguage, mylanguage[this.user.msId]);
      }
    },
    ...
    saveVoiceCallback (res) {
      if (res && res.voice) {
        ...
        obj.name = this.allMembersObj[res.msId].name
    ...
    isReciveNotice (res, name) {
      let remark = this.getRemark(res, name)
      this.$store.commit(log.SaveLog, {account: res.mid, name, type: res.type, remark})
      if (res.type=== codeConfig.repeatLoginNotice) {
        this.messageAlert('warning', remark)
        return
      }

      let notices = this.$store.getters.settingItems
      let info 
      if (notices) {
        for (let key in notices) {
          if (notices[key].key === noticeCode[res.type] && notices[key].val)
            info = `${name}${remark}`
        }
      }
      info && this.noticeAlert('info', info)
    },
    receiveGPSCallback (res) {
      ...
      this.saveAlarmNotice(res, lat, lng)
      if (lng == 0 && lat == lng) return
    ...
    initRightTempGroupList () {
      this.tempGroupList = this.hasLocalItems('myTempInfo') && Storage.localGet('myTempInfo')[this.user.msId]
    },
    hasLocalItems (item) {
      return Storage.localGet(item) && Storage.localGet(item)[this.user.msId]
    },
    getSendVoiceTime () {
      let sendTime
    ...
    getRemark (res) { // 通知备注
      return res.type === 8 ? `${this.languageLogType[res.type]} To ${res.gName}` : `${this.languageLogType[res.type]}`
    },
    throttleResize () {
    ...
    isAppLoading (val) {
      if (val) {
        ...
        this.loadingTimer = setTimeout(() => {
          this.$store.commit(app.SetAppLoading, false)
          this.messageAlert('warning', '网络超时')
        }, 8000)
      } else {
