(function (global) {
	...
	function SpeexDecoder(params) {
		...
		this.bits_size = params.bits_size !== undefined ? params.bits_size :
			SpeexEncoder.quality_bits[params.quality || 3];
	}
	...
	SpeexDecoder.prototype.init = function () {
		...
		this.state = state;
		this.bits = bits_addr;
		this.buffer = libspeex.allocate(this.frame_size, 'i16', libspeex.ALLOC_NORMAL);
	...
	SpeexDecoder.prototype.process = function (spxdata, segments) {
		...
		if (this.output.length < estimated_size) {
			this.output = this.floating_point ? new Float32Array(estimated_size) : new Int16Array(estimated_size);
		}
