<template>
  <div class="temp-wrap pr">
      <div class="notice pa w100 flex" v-if="isTipsShow && !tempGroupInfo">
      ...
      <Card class="card temp-card mt12" shadow :padding="10">
        ...
        <template v-else>
          <div class="member-wrap">
            <div class="member-list scroll-bar" :class="{active: tempGroupInfo}">
              <div class="flex delete-line sub-txt" v-if="!tempGroupInfo" @click="handleDeletMember('all')">
              ...
              <div
                ...
                <Icon type="ios-close" class="icon-close" size="20" @click="handleDeletMember(item)" v-if="!tempGroupInfo"/>
              </div>
            </div>
            <div class="btn-wrap flex between mt10">
              <Button class="btn theme-btn" size="small" v-if="!tempGroupInfo" @click="handleCreateTempGroup">{{languageCtx.tempGroupBtnText}}</Button>
...
export default {
  ...
  computed: {
    ...
    selectList () {
      let mapList = this.$store.getters.mapTempMemberList
      let tempList = this.$store.getters.groupTempList
      if (tempList.length) return tempList
      if (mapList.length) return mapList
      return []
    }
  },
  data () {
