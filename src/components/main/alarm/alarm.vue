  ...
  <div class="alarm-wrap flex align-center">
    <transition>
      <div class="list" v-if="alarmList.length">
          <div class="item">
            ...
            <span>{{languageCtx.alarmText}}</span>
          </Alert>
    ...
    <!-- <Modal 
       ...
       <p slot="header">
            ...
            <span class="modal-title">{{languageCtx.modalTitle}}</span>
        </p>
        <p class="f-black">{{alarmItem && alarmItem.name}}{{languageCtx.modalAlarmText}}</p>
        <div slot="footer">
            <Button type="primary" size="large" @click.stop="isAlarm=false">{{languageCtx.submitText}}</Button>
        </div>
    </Modal> -->
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    this.languageCtx = language[this.languageType].alarm
  },
  methods: {
    handleClose () {
